...
struct ImageData{
	Sprite bottom;   //background
	Sprite game_ground;  //logo
...
bool valiDate(int **m, int size, int row, int valuee)    //value check garna.....
{
...
bool validcolunm(int **m, int size, int column, int valuee)
{
	for(int i=0; i<size; i++)
	{
...
bool validDiagonal(int **m, int size,int row, int column, int valuee)
{
...
bool validTitle(int **m, int size,int row, int column, int valuee)         //for grid checking
{
	int quadx, quady;
...
bool reset(bool bol[16], int t)     //for resetting after error.....
{
	for(int i=0; i<t; i++)
	{
		if(bol[i])
...
int chooseDifficulty(RenderWindow &window, Data &media)
{
	...
	window.draw(media.images.bottom);
...
void selectTile(RenderWindow &window, int size,int &index, int **map, bool **bloc,Data &media, int &errors, bool diag, Entry &entry) //selecting block for entry..
{
	...
	if(!bloc[index/size][index%size])
	{
		if(Keyboard::isKeyPressed(Keyboard::Return))
		{
			...
			if(diag)
			{
				valid=validcolunm(map,size,index%size,entry.val) && valiDate(map,size,index/size,entry.val) && validTitle(map,size,index/size,index%size, entry.val) && validDiagonal(map,size,index/size,index%size, entry.val);
			}
			else
			{
				valid=validcolunm(map,size,index%size,entry.val) && valiDate(map,size,index/size,entry.val) && validTitle(map,size,index/size,index%size, entry.val);
			}
...
void dreaming(RenderWindow &window, TextData textt, int **map,bool **bloc, int size, int index, int time, int wrong, int coeficientDificulty, tiptype &tip)
{                                                                               //main game......
	...
	if(time%5==0)                                            //time for changing quotes..........
	{
		if(tip.exchange)
		{
			int aux=tip.tip;
	...
	string text="QUOTES: ";
	text+=textt.tips[tip.tip];
	Text tips(text, font, 20);
	//tips.setPosition(0, 550);
	tips.setPosition(400 - tips.getLocalBounds().width/2, 550);
	tips.setColor(Color::Blue);
	window.draw(errors);
	window.draw(added);
	window.draw(tips);

	RectangleShape nametitleg;                                      //for inner rows or grid
	nametitleg.setOutlineThickness(3);
	nametitleg.setSize(Vector2f(sizeTILE*quadx, sizeTILE*quady));
	nametitleg.setOutlineColor(sf::Color::Black);
	nametitleg.setOutlineColor(sf::Color(200,200,0));
	nametitleg.setFillColor(sf::Color::Transparent);

	RectangleShape nametitlefull;                                       //outer line
	nametitlefull.setSize(Vector2f(sizeTILE*size, sizeTILE*size));
	nametitlefull.setOutlineThickness(10);
	nametitlefull.setOutlineColor(sf::Color::Black);
	nametitlefull.setOutlineColor(sf::Color(200,200,0));
	nametitlefull.setFillColor(sf::Color::Transparent);
	nametitlefull.setPosition((400-size/2*sizeTILE), (300-size/2*sizeTILE));

	RectangleShape nametitle;                                               //inner lines
	...
	for(int i=0; i<size; i++)
	{
		for(int j=0; j<size; j++)
		{
			...
			{
				nametitleg.setPosition((400-size/2*sizeTILE)+j*sizeTILE,(300-size/2*sizeTILE)+i*sizeTILE);
				window.draw(nametitleg);
			}
		}
	}
	window.draw(nametitlefull);
}
		...
		for(int j=0; j<size; j++)
		{
			...
			if(i==j || i+j==size-1)
			{
				if(valbol[aux])
				{
					...
					valid = valiDate(m,size,i,val[aux])&& validcolunm(m,size,j,val[aux]) && validTitle(m,size,i,j,val[aux]) && validDiagonal(m,size,i,j,val[aux]);
					if(valid)
					{
						m[i][j]=val[aux];
						for(int i=0; i<16; i++)
	...
	for(int i = 0; i < size; i++)
	{
		for(int j = 0; j < size; j++)
		{
			...
			if(valbol[aux])
			{
				...
				{
					valid=valiDate(m,size,i,val[aux])&& validcolunm(m,size,j,val[aux]) && validTitle(m,size,i,j,val[aux]);
				}
...
void addDifficulty(int **map, bool **b, int size, int dificuldade)       //branch for difficulty(number of grids)
{
	int percentage;
	switch (dificuldade)
	{
...
void screensize(RenderWindow &window, Data &media, int dificuldade, int tam)   //tam=size of grid 6 or 9...
{
	...
	while(window.isOpen())
	{
		...
		window.draw(media.images.game_ground);
		dreaming(window,media.textts, m , b, size, index, time_drive, errors, coeficientDificulty, tip);
		...
		if(errors>10)      //how much errorrr to commite lose.....
		{
...
void drawButtons(RenderWindow &window, Data &media, int j, int index)
{
	...
	nametitlel.setOutlineThickness(2);
	nametitlel.setOutlineColor(Color::White);
	nametitlel.setFillColor(Color(255,0,0,100));
	nametitlel.setPosition(x,y+10);
	window.draw(nametitlel);
            media.images.button.setPosition(x,y);
			window.draw(media.images.button);
...
int main()
{
	...
	RenderWindow window(VideoMode(800, 600), "SUDOKUho", Style::Default);
